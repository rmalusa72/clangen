...
class Cat(object):
    used_screen = screen
    traits = ['adventurous', 'altruistic', 'ambitious', 'bloodthirsty', 'bold', 'calm', 'careful', 'charismatic', 'childish', 'cold', 'compassionate', 'confident', 'daring',
              'empathetic', 'faithful', 'fierce', 'insecure', 'lonesome', 'loving', 'loyal', 'nervous', 'patient', 'playful', 'responsible', 'righteous', 'shameless',
              'sneaky', 'strange', 'strict', 'thoughtful', 'troublesome', 'vengeful', 'wise']
    kit_traits = ['attention-seeker', 'bossy', 'bouncy', 'bullying', 'charming', 'daring', 'daydreamer', 'impulsive', 'inquisitive', 'insecure', 'nervous', 'noisy',
                  'polite', 'quiet', 'sweet', 'troublesome']
    ...
    skills = ['good hunter', 'great hunter', 'fantastic hunter',
    ...
    med_skills = ['good healer', 'great healer', 'fantastic healer',
                  'omen sight', 'dream walker', 'strong connection to StarClan',
                  'lore keeper', 'good teacher', 'great teacher',
                  'fantastic teacher', 'keen eye', 
                  'smart', 'very smart', 'extremely smart', 
    ...
    def thoughts(self):
        ...
        for c in self.all_cats.keys():
            ...
            if cat.dead:
                ...
                if other_cat.dead:
                    starclan_thoughts.extend(['Is sharing tongues with ' + other_name, 'Has been spending time with ' + other_name + ' lately', 'Is acting huffy at ' + other_name,
                ...
                # thoughts with other cats that are alive
                    ...
                    starclan_thoughts.extend(['Is watching over ' + other_name, 'Is curious about what ' + other_name + ' is doing', 'Wants to send a message to ' + other_name,
                ...
                # dead elder thoughts
                    ...
                    starclan_thoughts.extend(['Is grateful that they lived such a long life', 'Is happy that their joints no longer ache', 'Is telling stories to the younger cats of StarClan',
                ...
                if other_cat.dead:
                    ...
                    if cat.status in ['kitten', 'apprentice', 'medicine cat apprentice']:
                        thoughts.extend(['Is listening to stories about ' + other_name, 'Is learning more about ' + other_name, 'Is sad they couldn\'t spend time with ' + other_name,
                    ...
                    # older cat thoughts about dead cat
                        ...
                        thoughts.extend(['Is listening to stories about ' + other_name, 'Is learning more about ' + other_name, 'Is sad they couldn\'t spend more time with ' + other_name,
                    ...
                    # elder thoughts about dead cat
                        ...
                        thoughts.extend(['Was given a prophecy by ' + other_name, 'Was sent an omen by ' + other_name, 'Is dreaming about ' + other_name + ' who gives them a message',
                ...
                if cat.status == 'kitten':
                    thoughts.extend(['Plays mossball by themselves', 'Is annoying older cats', 'Wonders who their mentor will be', 'Wants to take a nap',
                                     'Tries to sneak out of camp', 'Is rolling around on the ground', 'Is chasing their tail',
                                     'Is playing with a stick', 'Is nervous for their apprentice ceremony', 'Is excited for their apprentice ceremony',
                                     'Is scared after having a nightmare', 'Wants to snuggle', 'Is asking older cats how kits are born',
                                     ...
                                     'Is batting pebbles across the camp clearing', 'Is crying after rough-housing too hard with the older cats',
                                     ...
                                     'Doesn\'t want to grow up yet...', 'Got in trouble for bringing thorns into the nest'])             
                    # checks for specific roles
                    if other_cat.status == 'elder':
                        thoughts.extend(['Was nipped on the rump by an elder for being naughty'])
                    ...
                    # kitten and skill specific thoughts
                        ...
                        thoughts.extend(['Thinks they saw a StarClan cat in their dreams', 'Is scrambling the medicine cat\'s herbs!', 'Is pretending to be the medicine cat'])                    
                ...
                # kitten and warrior apprentice thoughts    
                    ...
                    thoughts.extend(['Wonders what their full name will be', 'Pretends to be a warrior', "Practices the hunting crouch", 'Pretends to fight an enemy warrior',
                                     'Wants to be a warrior already!', 'Can\'t wait to be a warrior', 'Is pretending to be deputy', ])
                    # checks for specific roles
                    if other_cat.status == 'elder':
                ...
                # general apprentice thoughts
                    ...
                    thoughts.extend(['Is gathering moss', 'Is gossiping', 'Is acting angsty', 'Is dreading their apprentice duties',
                                     'Fell into the nearby creek yesterday and is still feeling damp', 'Is making their mentor laugh', 'Is really bonding with their mentor',
                                     'Is having a hard time keeping up with their training', 'Was tasked with lining nests with fresh moss today',
                                     'Is dreaming of someday making their Clan proud', ])
                    # checks for specific roles
                    if other_cat.status == 'kitten':
                        thoughts.extend(['Is awkwardly deflecting kits\' questions about where kits come from', 'Was put on kit-sitting duty', 'Is showing off to the kits',
                                         'Is telling off kits for being immature', 'Is rambling on to kits about the importance of respecting their elders'])
                    elif other_cat.status == 'elder':
                        thoughts.extend(['Was asked to gather fresh moss for the elders\' bedding', 'Plans to visit the elders soon', 'Is checking in on the elder\'s den',
                    ...
                    if cat.trait == 'adventurous':
                        thoughts.extend(['Is quietly trying to recruit other apprentices for a quick adventure'])
                    elif cat.trait == 'altruistic':
                        thoughts.extend(['Is thinking of giving their mentor a gift for their hard work', 'Made a keen suggestion to their mentor the other day'])
                        ...
                        thoughts.extend(['Starts a fight with another apprentice', 'Is hoping their warrior name will end in -claw'])
                    elif cat.trait == 'bold':
                        ...
                        thoughts.extend(['Winked playfully at another apprentice from across the clearing!', 'Is this moon\'s heartthrob to the other apprentices',
                                         'Has recently given a wonderful speech to fellow apprentices, boosting morale'])
                        # checks for specific roles
                        if other_cat.status == 'kitten':
                            thoughts.extend(['Has the kits very engaged in a very, very tall tale'])
                        elif cat.status == 'elder':
                        ...
                        thoughts.extend(['Is doing extra apprentice tasks around camp, to help lighten the load'])
                    elif cat.trait == 'insecure':
                        thoughts.extend(['Doesn\'t think that they have performing up to their mentor\'s standards lately...', ])
                    elif cat.trait == 'lonesome':
                        thoughts.extend(['Is feeling cramped in the apprentice\'s den'])
                    elif cat.trait == 'loving':
                        thoughts.extend(['Is listening to another apprentice\'s troubles sympathetically'])
                    elif cat.trait == 'loyal':
                        thoughts.extend(['Is listening to their mentor intently', 'Proclaimed to their mentor their unwavering loyalty'])
                    elif cat.trait == 'nervous':
                        thoughts.extend(['Is hoping to not train with their mentor today...', 'Wishes they were back in the nursery',
                                         'Has agreed to their mentor\'s orders recently, despite their own doubts', ])
                    elif cat.trait == 'playful':
                        thoughts.extend(['Won\'t stop making funny faces when their mentor\'s back is turned', 'Annoyed their mentor on accident the other day',
                                         'Successfully lightened a dreary mood while out training the other day',])
                    elif cat.trait == 'responsible':
                        thoughts.extend(['Is licking their chest in embarrassment after being praised by their mentor', 'Is asking their mentor what they can do to be helpful around camp today'])
                    elif cat.trait == 'righteous':
                        thoughts.extend(['Is refusing to follow their mentor\'s recent orders due to their own morals'])
                    elif cat.trait == 'shameless':
                        thoughts.extend(['Was found napping in the warrior\'s den!'])
                    elif cat.trait == 'strict':
                        thoughts.extend(['Is busy chastising fellow apprentices... but no cat is sure what for', 'Is participating in a rather rigorous training session'])
                    elif cat.trait == 'troublesome':
                        thoughts.extend(['Is ignoring their mentor\'s orders', 'Is making other apprentices laugh'])
                    elif cat.trait == 'vengeful':
                        thoughts.extend(['Snaps at another apprentice'])
                    elif cat.trait == 'wise':
                        thoughts.extend(['Is giving somber advice to a fellow apprentice', 'Was sought out by another apprentice recently for their wisdom'])
                    
                    # warrior apprentice specific thoughts
                    if cat.status == 'apprentice':
                        thoughts.extend(['Is thinking about the time they caught a huge rabbit', 'Practices some battle moves', 'Is helping to reinforce the camp wall with brambles',
                        ...
                        if cat.trait == 'ambitious':
                            thoughts.extend(['Begs to be made a warrior early', 'Seems to be ordering their fellow apprentices around',
                                             'Has been catching the most prey out of all the apprentices'])
                        elif cat.trait == 'bloodthirsty':
                            thoughts.extend(['Pesters their mentor about doing battle training', 'Is thinking about murder', 'Draws blood during their battle training', ])
                        elif cat.trait == 'cold':
                            thoughts.extend(['Is hoping their warrior name will end in -claw'])
                        elif cat.trait == 'bold':
                            thoughts.extend(['Is criticizing their mentor', 'Taunted rival Clan apprentices at the border the other day',
                                             'Is looking to challenge a warrior to a sparring match'])
                        elif cat.trait == 'confident':
                            thoughts.extend(['Is sure that they\'ll be made into a warrior today'])
                        elif cat.trait == 'daring':
                            thoughts.extend(['Is being scolded by the deputy for reckless behavior while out training'])
                        elif cat.trait == 'fierce':
                            thoughts.extend(['Is showing off their new battle moves','Is pushing hard for more battle training',
                                             'Was recently chastised by their mentor for reckless behaviour out on patrol', 'Practiced battle moves with their claws out'])
                        elif cat.trait == 'insecure':
                            thoughts.extend(['Is wondering if they are good enough to be a warrior...', 'Wonders if the medicine cat life would have better suited them...',
                                             'Is reluctant to spar with their mentor today', 'Doesn\'t think their hauls on hunting patrols have been substantial enough as of late'])
                        elif cat.trait == 'nervous':
                            thoughts.extend(['Hopes that they will be a strong enough warrior...', 'Is wondering if they are more suited for life as a medicine cat...',
                                             'Was startled by a squirrel while out training!',])
                        elif cat.trait == 'thoughtful':
                            thoughts.extend(['Offered to go on the dawn patrol with their mentor'])                     
                
                    # medicine cat apprentice specific thoughts
                    if cat.status == 'medicine cat apprentice':
                        thoughts.extend(['Is wondering if they are good enough to become a medicine cat', 'Wishes the other apprentices could understand how they feel',
                                         'Helps apply a poultice to a small wound', 'Is enjoying learning all of the herbs a medicine cat needs!'])
                        # checks for specific roles
                        if other_cat.status == 'kitten':
                            thoughts.extend([])
                        elif other_cat.status == 'elder':
                        ...
                        if cat.trait == 'cold':
                            thoughts.extend(['Is hoping their medicine cat name will end in -claw'])
                        elif cat.trait == 'insecure':
                            thoughts.extend(['Is wondering if they are good enough to be a medicine cat...',
                                             'Wonders if the warrior life would have better suited them...'])
                        elif cat.trait == 'nervous':
                            thoughts.extend(['Is wondering if they are more suited for life as a warrior...'])                    
                ...
                if cat.status == 'medicine cat' or cat.status == 'medicine cat apprentice':
                    thoughts.extend(['Is looking for herbs', 'Is organizing the herb stores', 'Is drying some herbs', 'Is counting the poppy seeds', 'Is gathering cobwebs',
                                     ...
                                     'Is feeling stressed taking care of the Clan', 'Is wondering if they could borrow some catmint from the other Clans',
                                     'Is wrapping a wound with cobwebs', 'Is clearing out old herbs', 'Is tending growing herbs', 'Is making new nests',
                                     ...
                                     'Plans to go out gathering herbs today', 'Is looking forward to the half-moon meeting',
                                     'Is struggling to remember all of the names of herbs', 'Is helping organize the herb stores',
                                     'Is proud of their ability to care for their Clanmates','Made a mess of the herbs and is panicking',
                                     'Has been hearing the voices of StarClan cats...', 'Has the foul taste of bitter herbs in their mouth',
                                     'Is happy that they chose life as a medicine cat', 'Is lining nests with fresh moss and feathers'])
                    # medicine cat only thoughts no apprentices allowed
                    if cat.status != 'medicine cat apprentice':
                        thoughts.extend(['Is thinking about taking on a new apprentice'])
                    # checks for specific roles
                    if cat.status == 'kitten':
                        thoughts.extend(['Is teaching kits about what plants to stay away from', 'Chased kits out of their den', ])   
                        # roles + traits
                        if cat.trait == 'bloodthirsty':
                            thoughts.extend(['Encourages kits to eat some strange red berries'])
                    ...
                    if cat.trait == 'adventurous':
                        ...
                        thoughts.extend(['Is gathering deathberries', 'Has been disappearing a lot lately', 'Insists only on treating cats who need it', 'Is ripping some leaves to shreds',
                                         'Debates becoming a warrior', 'Gives the wrong herbs to a warrior on purpose'])
                    elif cat.trait == 'bold':
                        ...
                        thoughts.extend(['Insists everyone eat chamomile leaves everyday at moonhigh', 'Sleeps in the middle of the clearing', 'Looks dazed',
                ...
                if cat.status == 'deputy':
                    thoughts.extend(['Is assigning cats to a border patrol', 'Is assigning cats to a hunting patrol', 'Is wondering what it would be like to be a leader', 
                                     'Is stressed about organizing patrols', 'Wonders who will give them nine lives', 'Feels overworked', 'Is hoping for a break',
                                     'Is assessing the apprentices', 'Wishes they had an extra set of paws', 'Is assigning cats to the dawn patrol',
                                     'Is assigning cats to the hunting patrol', 'Is assigning cats to patrol the borders', 'Can\'t believe they overslept today',
                    ...
                    if cat.trait == 'bloodthirsty':
                        thoughts.extend(['Thinks about killing the leader and staging it as an accident', 'Encourages the leader to start a war'])
                    elif cat.trait == 'strange':
                ...
                if cat.status == 'leader':
                        thoughts.extend(['Is hoping for a sign from StarClan', 'Is hoping that they are leading their Clan well', 'Thinks about who should mentor new apprentices',
                                         'Is worried about Clan relations', 'Tries to set a good example for the deputy', 'Is assessing some apprentices', 
                                         'Is thinking about forming an alliance', 'Is thinking about battle strategies', 'Almost lost a life recently',
                        ...
                        # checks for specific roles
                        if other_cat.status == 'kitten':
                            thoughts.extend(['Has recently picked up the scent of mischievous kits in their den...'])
                ...
                if cat.status == 'warrior':
                    ...
                    if cat.trait == 'ambitious':
                        thoughts.extend(['Is asking the Clan leader what they can do to help out around camp', 'Has been imitating the Clan leader\'s behaviour recently',
                                      'Envies the deputy\'s position', ])
                    elif cat.trait == 'fierce':
                        thoughts.extend(['Was recently chastised by the deputy for reckless behaviour out on patrol'])
                    elif cat.trait == 'loyal':
                        thoughts.extend(['Is listening to the Clan leader intently', 'Is listening to the deputy intently', 'Is telling the Clan leader details about the recent patrol',
                                         ...
                                         'Has agreed to their Clan leader\'s orders recently, despite their own doubts', 'Proclaimed to the Clan leader their unwavering loyalty'])
                    elif cat.trait == 'righteous':
                ...
                if cat.status == 'warrior':
                    thoughts.extend(['Caught scent of a fox earlier', 'Caught scent of an enemy warrior earlier', 'Is helping gathering herbs', 'Is thinking about love',
                                     ...
                                     'Is sparring with some Clanmates', 'Fell into the nearby creek yesterday and is still feeling damp', 'Is guarding the camp entrance',
                    ...
                    if other_cat.status == 'kitten':
                        ...
                        if cat.trait == 'cold':
                            thoughts.extend(['Recently snapped at the kits, making them cry'])
                        elif cat.trait == 'childish':
                            thoughts.extend(['Is teaching new games to the kits'])
                        elif cat.trait == 'empathetic':
                            thoughts.extend(['Is comforting kits after a scary experience'])
                        elif cat.trait == 'fierce':
                            thoughts.extend(['Is roaring playfully at kits, making them laugh', 'Has been rough housing with the kits a little too hard lately'])
                        elif cat.trait == 'loving':
                            thoughts.extend(['Is helping out with kits around camp'])
                        elif cat.trait == 'playful':
                            thoughts.extend(['Is giving kits badger rides on their back!', 'Is riling up the kits, much to the queens\'s dismay'])
                        elif cat.trait == 'shameless':
                            thoughts.extend(['Pushed a kit out of their way thoughtlessly'])
                        elif cat.trait == 'strict':
                            thoughts.extend(['Is grumbling about troublesome kits'])
                        elif cat.trait == 'thoughtful':
                            thoughts.extend(['Is bringing soaked moss to the queens in the nursery', 'Is promising to take the kits out on a stroll today if they behave',
                                     'Plucked feathers from their meal for the kits to play with', 'Is offering to look after the kits while the queens rest'])
                        elif cat.trait == 'troublesome':
                            thoughts.extend(['Got scolded for telling the kits a naughty joke!'])   
                    elif other_cat.status == 'elder':
                        if cat.trait == 'calm':
                            ...
                            thoughts.extend(['Helped the elders to rise stiffly from their nests this morning'])                            
                        elif cat.trait == 'empathetic':
                            thoughts.extend(['Volunteered to gather fresh lining to the elders\' nests'])                            
                        elif cat.trait == 'loyal':
                            thoughts.extend(['Is checking in on the elder\'s den', 'Is rambling on to younger cats about the importance of respecting their elders'])
                        elif cat.trait == 'thoughtful':
                            thoughts.extend(['Gave an elder their favorite piece of fresh kill', 'Is making sure that the elders all have fresh bedding'])                            
                        elif cat.trait == 'troublesome':
                            thoughts.extend(['Recently was scolded for eating prey before the queens and elders'])
                    elif other_cat.status == 'apprentice':
                        thoughts.extend(['Has the apprentices very engaged in a very, very tall tale'])
                        if cat.trait == 'childish':
                            thoughts.extend(['Is pouncing on unsuspecting apprentices'])                            
                        elif cat.trait == 'cold':
                            thoughts.extend(['Is scolding the apprentices over something slight'])
                        elif cat.trait == 'thoughtful':
                            thoughts.extend(['Is hosting a modified training session for the beginner apprentices'])
                    ...
                    if cat.trait == 'adventurous':
                        thoughts.extend(['Is itching to explore the land beyond their Clan\'s territory', 'Wants to go on a journey', 'Wishes their leader would choose them to go on a quest'])
                    elif cat.trait == 'altruistic':
                        thoughts.extend(['Offered to walk at the front of the patrol', 'Offered to stick their nose inside a badger set to see if the badger was still there',
                                         'Offered to stick their nose inside a fox den to see if the fox was still there', 'Volunteered for extra patrols', 'Volunteered for extra duties',
                                         'Volunteered to stand guard of the camp'])
                    elif cat.trait == 'ambitious':
                        thoughts.extend(['Is boasting loudly about having defeated an enemy warrior on patrol the other day',
                                         'Has been taking on extra patrols lately',])
                    elif cat.trait == 'bold':
                        thoughts.extend(['Is getting some looks after speaking up at the last Clan meeting', 'Spoke up recently at a Clan meeting'])
                    elif cat.trait == 'careful':
                        thoughts.extend(['Is dutifully standing guard outside of camp', 'Is helping to reinforce the nursery walls',
                                     'Is helping to reinforce the camp walls', 'Is patching up a hole in the camp wall', 'Is helping to reinforce the walls of the elders\' den',
                                     'Is going back to check out an old fox burrow they discovered yesterday, just to be safe', 'Is going back to check out an old badger set they discovered yesterday, just to be safe'])
                    elif cat.trait == 'childish':
                        thoughts.extend(['Splashes in a puddle of water during a patrol', 'Jumps around while on patrol',])
                    elif cat.trait == 'confident':
                        thoughts.extend(['Boasts about how much fresh kill they intend to bring back to camp today', 'Thinks that they are the best hunter in the Clan',
                                         'Thinks that they are the fastest runner in the Clan', 'Is showing off their battle moves', 'Thinks that they are the fiercest fighter in the Clan'])
                    elif cat.trait == 'daring':
                        thoughts.extend(['Batted at a snake on a patrol recently and fled', 'Is leaping from boulder to boulder out in the forest', 
                                         'Is climbing to the top of a very tall tree!', 'Recently avoided a monster on the Thunderpath by the skin of their teeth!'])
                    elif cat.trait == 'faithful':
                        thoughts.extend(['Is thanking StarClan for their catch out on a hunting patrol today'])
                    elif cat.trait == 'fierce':
                        thoughts.extend(['Is asking with the medicine cat what herbs may help them to be stronger in battle',
                                         'Is showing off their battle moves', 'Is offering to lead the patrol today'])
                    elif cat.trait == 'insecure':
                        thoughts.extend(['Almost died of embarrassment after a recent fumble on a patrol'])
                    elif cat.trait == 'loving':
                        # checks for specific roles
                        if other_cat.status == 'kitten':
                            thoughts.extend(['Is helping out with kits around camp', 'Is smiling at the antics of the kits', 'Is offering words of comfort to the kits'])                        
                    elif cat.trait == 'nervous':
                        thoughts.extend(['Was startled by a shrew while out on patrol!', 'Was startled by a fluttering bird while out on patrol!',
                                         'Acted bravely on a recent patrol, despite their anxieties', ])
                    elif cat.trait == 'playful':
                        # checks for specific roles
                        if other_cat.status == 'kitten':
                            thoughts.extend(['Is giving kits badger rides on their back!', 'Is showing kits a game they used to play when they were that age', 
                                             'Is riling up the kits, much to the queens\' dismay'])                        
                    elif cat.trait == 'responsible':
                        thoughts.extend(['Is repairing one of the camp walls', 'Is offering to lead the next patrol out'])
                    elif cat.trait == 'strict':
                        thoughts.extend(['Can\'t stand to watch the younger cats make fools of themselves',
                                         'Is conducting a rather rigorous training session for the younger warriors',
                                         'Takes pride in how well-run their patrols always are', 'Is telling off younger cats for petty dishonesty out on patrol'])
                    elif cat.trait == 'thoughtful':
                        thoughts.extend(['Brought back a much-needed herb to a grateful medicine cat after a hunting patrol'])
                    elif cat.trait == 'troublesome':
                        thoughts.extend(['Is grumbling as they carry out a task for the medicine cat', 'Climbed up a tree and nearly fell out!'])                
                ...
                if cat.trait == 'adventurous':
                    ...
                    thoughts.extend(['Is laughing at a Clanmate\'s joke, even though they didn\'t find it too terribly funny', 'Helped to gather herbs all day with the medicine cat'])
                    # checks for specific roles
                    if other_cat.status == 'elder':
                        thoughts.extend(['Is taking fresh kill to the elders and queens', 'Gave their share of fresh kill to the elders',
                                         'Is putting mousebile on the elder\'s ticks'])
                    elif other_cat.status == 'kitten':
                        thoughts.extend(['Is following the kits around camp, giving the queens a break', 'Let the kits sleep in their nest with them last night',
                                         'Is grooming the scruffiest kits around camp dutifully'])
                elif cat.trait == 'ambitious':
                    thoughts.extend(['Is being admired by many Clanmates for their recent feats', 'Is boasting loudly about having defeated an enemy warrior on patrol the other day',
                                    'Seems to be ordering their fellow Clanmates around', 'Tries to put on a brave face for their Clanmates', 'Is feeling proud of themselves',
                                    'Made sure to wake up early to volunteer for the dawn patrol', 'Has been catching the most prey on the latest hunting patrols', 
                                    'Is daydreaming about a Clan celebration in their honor someday'])
                elif cat.trait == 'bloodthirsty':
                    thoughts.extend(['Is acting suspicious', 'Has been washing their paws a lot lately', 'Growls to themselves', 'Has disappeared often recently',
                                     'Is thinking about murder', 'Tells the warriors to get ready for a battle', 'Yearns to fight', 'Daydreams about killing a certain cat',
                                     'Wonders if they could kill a fox all by themselves', 'Started a fight with a fellow Clanmate'])
                elif cat.trait == 'calm':
                    thoughts.extend(['Is taking a moment to appreciate some peace and quiet', 'Looks on as the camp buzzes around them',
                    ...
                    thoughts.extend(['Is double-checking their nest for burrs', 
                    ...
                    thoughts.extend(['Is chasing a butterfly around the camp', 'Is playing with their food',
                                         'Is giggling about Clan drama with friends', 'Is crunching leaves with their paws', 'Is whining about Clan duties',
                                         'Is distracted by a shiny Twoleg trinket they found', 'Is gnawing on a small bone busily',
                                         'Is batting at a ball of moss', 'Is preoccupied playing an important game of hide and seek', 'Is laughing gleefully!'])
                    # checks for specific roles
                    if other_cat.status == 'kitten':
                        thoughts.extend(['Is pouncing on unsuspecting kits', 'Is teaching new games to the kits'])
                elif cat.trait == 'cold':
                    thoughts.extend(['Is hissing in frustration', 'Is grunting rudely at passersby',
                    ...
                    thoughts.extend(['Is being scolded for giving their prey away to a starving loner', 'Spent time today with a grieving Clanmate',
                                     ...
                                     'Is noticing with joy how well the Clan is looking after one another as of late', 'Is listening to a Clanmate\'s struggles with love'])
                    # checks for specific roles
                    if other_cat.status == 'elder':
                        thoughts.extend(['Helped the elders to rise stiffly from their nests this morning'])
                elif cat.trait == 'confident':
                    thoughts.extend(['Is building up a fellow Clanmate\'s confidence in battle!',
                                     'Is puffing their chest out', 'Thinks that they are the smartest cat in the Clan',
                                     'Thinks that they are the funniest cat around', 'Is strutting around confidently'])
                    if cat.status != 'deputy' and cat.status != 'leader':
                        thoughts.extend(['Is letting the Clan leader know their opinion on a rather serious matter', 'Knows without a doubt that the Clan deputy respects them',
                                         'Knows without a doubt that the Clan leader must respect them', 'Is sure to stand tall when the Clan leader walks by'])
                elif cat.trait == 'daring':
                    thoughts.extend(['Is racing through camp, accidentally knocking into Clanmates', 'Suffered a bellyache after being dared to swallow a beetle',
                    ...
                    thoughts.extend(['Isn\'t scared of anything today!', 'Can\'t stop flexing their claws',
                                     'Woke up with a flame in their belly this morning!', 'Is unusually tame today', 'Is looking forward to a challenge',
                                     'Thinks that they could take on a badger solo', 'Thinks that they could take on a fox solo',
                    ...
                    thoughts.extend(['Is wondering if they are good enough...', 'Is thinking about where they belong', 'Is hoping no cat saw them trip just now',
                                     'Hopes that they will make it through leaf-bare', 'Took their own insecurities out on a friend the other day and feels awfully guilty...'])
                elif cat.trait == 'lonesome':
                    ...
                    thoughts.extend(['Is determined to protect their loved ones', 'Is making sure that everyone has eaten well', 'Is boasting about their loyalty to the Clan',
                                     'Recently reported the nearby scent of rogues in the territory', 'Is determined to protect their loved ones, now moreso than ever',
                                     'Is making sure that everyone has eaten well', 'Recently reported the nearby scent of rogues in the territory',
                                     'Is boasting about their loyalty to the Clan'])  
                    # checks for specific roles
                    if other_cat.status == 'kitten':
                        thoughts.extend(['Is roaring playfully at kits, making them laugh', 'Has been rough housing with the kits a little to hard lately',
                                        'Is telling the kits tales about valiant warriors in the thick of epic battles'])
                elif cat.trait == 'nervous':
                    thoughts.extend(['Feels like there are ants pricking their pelt...', 'Is dreading the thought of failure...', 'Is nervously pacing around camp',
                                     'Is nervously glancing around camp', 'Thinks that they are hearing things...',
                    ...
                    thoughts.extend(['Is waiting until everyone else has taken from the fresh kill pile to eat', 'Is patiently standing guard outside of camp',
                                         'Is winning a staring contest against a Clanmate', 'Is listening to a story they\'ve heard many, many times',
                    ...
                    thoughts.extend(['Is playing tag with cats around camp', 'Is tossing mossballs around', 'Pounced on a stray leaf with satisfaction',
                                     'Seems to be playing with their food', 'Is play-fighting with friends', 'Is playing a games of counting stones',
                    ...
                    thoughts.extend(['Is making sure they\'ve done all of their daily duties', 'Is planning to wake up early tomorrow',
                                     'Was the first to rise this morning', 'Roused the other cats awake this morning',
                    ...
                    # checks for specific roles
                    if other_cat.status == 'elder':
                        thoughts.extend(['Is going to fetch the elders new bedding today'])
                    elif other_cat.status == 'kitten':
                        thoughts.extend(['Is making sure the kits behave'])
                elif cat.trait == 'righteous':
                    thoughts.extend(['Wants nothing more than to live by the Warrior Code', 'Is praying to StarClan for guidance...', 'Is reevaluating their morals',
                    ...
                    if other_cat.status == 'kitten':
                        thoughts.extend(['Is teaching kits how to walk without making a sound'])
                elif cat.trait == 'strange':
                    thoughts.extend(['Is telling every cat about their weird dreams', 'Is chasing a butterfly', 'Stares at random cats', 'Volunteered to guard the dirtplace...?',
                                     ...
                                     'Is making odd noises', 'Is tearing up their food', 'Is gnawing on a stick', 'Collects the bones of prey', 'Is staring intently at a wall',
                    ...
                    thoughts.extend(['Is lecturing any cat who will listen about the Warrior Code', 'Is lashing their tail furiously',
                    ...
                    # checks for specific roles
                    if other_cat.status == 'kitten':
                        thoughts.extend(['Is grumbling about troublesome kits', 'Can\'t stand to watch the kits make fools of themselves'])
                elif cat.trait == 'thoughtful':
                    thoughts.extend(['Offered to fetch more herbs for the medicine cat', 'Is offering helpful advice to a gloomy Clanmate'])
                    # checks for specific roles
                    if other_cat.status == 'elder':
                        thoughts.extend(['Gave an elder their favorite piece of fresh kill', 'Is making sure that the elders all have fresh bedding'])
                    elif other_cat.status == 'queen':
                        thoughts.extend(['Is bringing soaked moss to the queens in the nursery'])
                    elif other_cat.status == 'kitten':
                        thoughts.extend(['Is promising to take the kits out on a stroll today if they behave', 'Plucked feathers from their meal for the kits to play with',
                                            'Is hosting a mock training session for the kits', 'Is offering to look after the kits while the queens rest'])
                elif cat.trait == 'troublesome':
                    thoughts.extend(['Won\'t stop pulling pranks', 'Is causing problems', 'Recently put a dead snake at the camp entrance to scare Clanmates', 
                    ...
                    thoughts.extend(['Seems to be plotting something', 'Is definitely plotting something', 'Is glaring daggers across the camp clearing',
                                     ...
                                     'Is angrily clawing up the ground, lost in deep thought', 'Is shredding the grass underpaw'])
                elif cat.trait == 'wise':
                    thoughts.extend(['Has a suggestion for the Clan leader that they wish to present', 'Is grooming themselves thoughtfully',
                                     'Is telling stories to a very interested bunch of apprentices'])
                    # checks for specific roles
                    if other_cat.status == 'kitten':
                        thoughts.extend(['Is teaching kits how to identify prey prints in the dirt', 'Is counseling the kits'])                
